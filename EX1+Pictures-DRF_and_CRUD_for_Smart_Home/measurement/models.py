...
class Measurement(models.Model):
    ...
    #====дОПОЛНИТЕЛЬНО ДЛЯ ХРАНЕНИЯ КАРТИНОК С ДАТЧИКОВ

    image = models.ImageField(
        upload_to='measurements/PICTURES/',  # файлы будут складываться в MEDIA_ROOT/measurements/
        null=True,  # в БД может быть NULL
        blank=True,  # форма/сериализатор не требуют обязательного значения
    )
    # ====================================
    def __str__(self):
